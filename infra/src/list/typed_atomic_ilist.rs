...
impl<T, A: Adapter<T>> AtomicListHead<T, A> {
    ...
    #[inline]
    pub fn list_head_of(this: &T) -> &mut Self {
        let ptr: *mut u8 = unsafe { core::mem::transmute(this as *const _ as *const u8) };
...
mod tests {
    ...
    fn test_concurrent_ops1() {
        ...
        for i in 0..n {
            ...
            let t = thread::spawn(move || {
                let lh = unsafe { Ty::list_head_of(Arc::<Foo>::get_mut_unchecked(&mut f)) };
                for k in 0..256 {
                    {
                        let mut h = head.write();
