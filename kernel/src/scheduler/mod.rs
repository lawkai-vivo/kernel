...
use crate::{
    ...
    types::{Arc, IlistHead, ThreadPriority, Uint},
};
use alloc::boxed::Box;
...
pub(crate) extern "C" fn relinquish_me_and_return_next_sp(old_sp: usize) -> usize {
    ...
    let Some(next) = next_preferred_thread(current_thread_priority()) else {
        #[cfg(debugging_scheduler)]
...
fn yield_unconditionally() {
    ...
    let to_sp = next.saved_sp();
    let from_sp_ptr = current_thread_saved_sp_ptr();
...
pub fn relinquish_me() {
    debug_assert!(arch::local_irq_enabled());
    let prio = current_thread_priority();
    let Some(next) = next_preferred_thread(prio) else {
        return;
    };
    let to_sp = next.saved_sp();
    let from_sp_ptr = current_thread_saved_sp_ptr();
...
#[inline]
pub fn current_thread_saved_sp_ptr() -> *const u8 {
    let _guard = DisableInterruptGuard::new();
    let my_id = arch::current_cpu_id();
    let t = unsafe { RUNNING_THREADS[my_id].assume_init_ref() };
    t.saved_sp_ptr()
