...
pub fn add_hard_timer(tm: &mut Timer) -> Option<Iou<'_>> {
    let mut iou;
    {
        ...
        //semihosting::println!("Add: Before: Num hwtm: {}", w.size());
        iou = w.add(tm);
        //semihosting::println!("Add: After: Num hwtm: {}", w.size());
    }
    update_clock_interrupt();
    iou
}
...
pub fn remove_hard_timer<'a>(iou: Iou<'_>) -> Option<Iou<'a>> {
    ...
    {
        let mut w = HW_TIMERS.irqsave_lock();
        //semihosting::println!("Remove: Before: Num hwtm: {}", w.size());
        res = w.remove(iou);
        //semihosting::println!("Remove: After: Num hwtm: {}", w.size());
    }
    update_clock_interrupt();
    res
}
...
pub(crate) fn expire_timers(deadline: Tick) -> Option<Tick> {
    ...
    {
        let mut w = HW_TIMERS.irqsave_lock();
        //semihosting::println!("Expire at {:?}: Before: Num hwtm: {}", deadline, w.size());
        w.expire(deadline);
        //semihosting::println!("Expire at {:?}: After: Num hwtm: {}", deadline, w.size());
        w.post_expire();
        hard_deadline = w.next_deadline().unwrap_or(Tick::MAX);
        res = core::cmp::min(soft_deadline, hard_deadline);
    }
    //    semihosting::println!(
    //        "Expired timers on CPU#{} at {:?}",
    //        arch::current_cpu_id(),
    //        deadline
    //    );
...
mod tests {
    ...
    fn test_periodic_swtm() {
        ...
        //semihosting::println!("Base deadline: {:?}", base_deadline);
        let r = Repeat {
            base_deadline,
            period: Tick(10),
            total_times: Some(3),
    ...
    fn test_timer_edge_cases() {
        ...
        let callback = create_test_callback(&counter);
        let mut timer = Timer::new();
        timer.mode = TimerMode::Deadline(Tick::after(Tick(0)));
